            ...
            steps {
                ...
                sh '''#!/bin/bash
                    ...
                    if echo $DOCKERHUB_CREDENTIALS_PSW | docker login -u $DOCKERHUB_CREDENTIALS_USR --password-stdin;then
                        ...
                        if docker build . -t muhanedyahya/onlinecelebration-php:v5 -f dockerfiles/php.dockerfile --no-cache;then
                            echo "php image successfully created.";
                            echo "pushing php image to DockerHub.....";
                                if docker push muhanedyahya/onlinecelebration-php:v5;then
                        ...
                        if docker build . -t muhanedyahya/onlinecelebration-nginx:v5 -f dockerfiles/nginx.dockerfile --no-cache;then
                            echo "nginx image successfully created.";
                            echo "pushing nginx image to DockerHub.....";
                                if docker push muhanedyahya/onlinecelebration-nginx:v5;then
